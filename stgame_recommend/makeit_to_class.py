...
class CollaborateFiltering:
    def __init__(self, k=5):
        ...
        self.clean_df = self.df.drop_duplicates(['UserID', 'Game'], keep='last').drop(['Action', 'Hours', 'Other'],
                                                                                      axis=1)
        ...
        self.game_idx_origin = self.clean_df['gameIdx'] = self.clean_df['Game'].apply(
            lambda x: self.game2idx_origin[x]).values
        self.hours_origin = self.clean_df['Hours_Played'].values

        self.zero_matrix_origin = np.zeros(
            shape=(self.n_users_origin, self.n_games_origin))  # Create a zero matrix(유저수x게임수)
    ...
    def eval_result(self):
        ...
        print('Precision of {0}'.format(
            len(set(new_recommendations) & set(np.where(self.test_matrix[self.user2idx[0], :] != 0)[0])) / float(
                self.k)))
        print('--------------------------------------')
    ...
    def train_data(self):
        ...
        self.train_matrix, self.val_matrix = data_process(self.user_game_pref, self.train_matrix, self.val_matrix,
                                                          val_users_idx, self.k)
        self.train_matrix, self.test_matrix = data_process(self.user_game_pref, self.train_matrix, self.test_matrix,
                                                           test_users_idx, self.k)
        ...
        while (stop_signal == 0):
            ...
            for i in range(1, iterations):
                ...
                if ((i % 10 == 0) or (i == 1)):
                    mod_loss = sess.run(loss,
                                        feed_dict={pref: self.train_matrix, interactions: self.user_game_interactions})
                    if (mod_loss < 0):
                        break
